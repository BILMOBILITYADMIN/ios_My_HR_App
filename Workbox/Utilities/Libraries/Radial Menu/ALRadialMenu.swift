...
public class ALRadialMenu: UIButton {
    ...
    public func setButtons(buttons: [ALRadialMenuButton]) -> Self {
        ...
        for i in 0..<buttons.count {
            ...
            button.action = {
                self.dismiss(i)
                switch i {
                case 0: self.delegate?.getButtonIndex(i)
                print("Call ")
                
                let phoneNumber = String(button.titleLabel?.text ?? "")
                if Helper.lengthOfStringWithoutSpace(phoneNumber) > 6{
                    if let url = NSURL(string: "tel://\(phoneNumber)"){
                        if UIApplication.sharedApplication().canOpenURL(url){
                            ...
                            if let topVC = getTopViewController(){
                                ...
                                let alertController = UIAlertController.init(title: "Action not possible", message: "Phone Call is not supported", preferredStyle: UIAlertControllerStyle.Alert)
                    ...
                    if let topVC = getTopViewController(){
                        let alertController = UIAlertController.init(title: "Action not possible", message: "Phone Number of the user not available", preferredStyle: UIAlertControllerStyle.Alert)
                        let okAction = UIAlertAction.init(title: "OK", style: UIAlertActionStyle.Cancel, handler: nil)
                        alertController.addAction(okAction)
                        topVC.presentViewController(alertController, animated: true, completion: nil)
                    }
                ...
                case 1:self.delegate?.getButtonIndex(i)
                self.delegate?.messageAction()
                print("Message")
                    
                case 2: self.delegate?.getButtonIndex(i)
                self.delegate?.mailAction()
                print("Mail 2")
                    
                default: self.delegate?.getButtonIndex(i)
                }
            }
        }
    ...
    */
    public func presentInWindow(win: UIWindow) -> Self {
        ...
        if buttons.count == 0 {
            print("ALRadialMenu has no buttons to present", terminator: "")
            return self
        }
    ...
    */
    public func dismiss() -> Self {
        ...
        if buttons.count == 0 {
            print("ALRadialMenu has no buttons to dismiss", terminator: "")
            return self
        }
    ...
    private func commonInit() {
        dismissGesture = UITapGestureRecognizer(target: self, action: #selector(ALRadialMenu.dismiss(_:)))
        dismissGesture.enabled = dismissOnOverlayTap
        overlayView.addGestureRecognizer(dismissGesture)
        overlayView.backgroundColor = UIColor.init(white: 0.0 , alpha: 0.8)
    }
    
    @objc private func dismiss(selectedIndex: Int) {
